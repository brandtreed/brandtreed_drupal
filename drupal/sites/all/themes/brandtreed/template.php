...
if (theme_get_setting('brandtreed_rebuild_registry')) {
...
function phptemplate_preprocess_node(&$vars, $hook) {
  ...
  switch ($node->type) {
    case 'blog':
      $vars['submitted'] = brandtreed_blog_date($node);
      break;
  }
}
...
function phptemplate_preprocess_comment(&$vars, $hook) {
  ...
  $vars['comment_date'] = brandtreed_comment_date($comment);
 ...
 * @return themed date.
 */
function brandtreed_blog_date($node) {
 ...
 * @return themed date.
 */
function brandtreed_comment_date($comment) {
...
*/
function brandtreed_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
...
/**
* Removes the words "All Day" on all CCK Date fields
*/
function brandtreed_date_all_day_label() {
  return '';
}
